...
└── homeworks/
    ...
    └── project/
        ├── code/                  # 模型实现与引用记录
        ├── configs/               # 可复现实验配置
        ├── experiments/           # 实验过程与指标摘要
        ├── scripts/               # 本地、远端与评估脚本
        ├── docs/public/           # 可公开的设计与资源说明
...
课程 starter code 尚未合入时，不在仓库根目录预先固定依赖。引入后以其 lockfile 和 setup 脚本为准，并记录 PyTorch、CUDA 与驱动版本。正式实验使用自动混合精度，通过 commit、配置文件和随机种子保证可复现性。

计算预算与双 GPU 执行策略见 [`homeworks/project/docs/public/compute-resources.md`](homeworks/project/docs/public/compute-resources.md)。
